...
class GeometryLayerFromGeojsonStringField(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(
            QgsProcessingParameterFeatureSource(
                self.SOURCE_LYR, self.tr('Source'), [QgsProcessing.TypeVectorAnyGeometry])) # Take any source layer, unfortunately no-geometry layers will not be available...
        self.addParameter(
            QgsProcessingParameterField(
                self.GEOJSON_FIELD, self.tr('Field containing the GeoJSON as string'),'GeoJSON','SOURCE_LYR')) # Choose the field containing the GeoJSON as string
        self.addParameter(
            QgsProcessingParameterNumber(
                self.GEOMETRYTYPE_STRING, self.tr('wkb type of geometry (lookup at https://qgis.org/pyqgis/3.0/core/Wkb/QgsWkbTypes.html)'),0,5)) # Unfortunately there is no WKB-Type-Input available...
        self.addParameter(
            QgsProcessingParameterCrs(
                self.CRS, self.tr('CRS of the GeoJSON content'),'EPSG:4326')) # CRS of the targetlayer
        self.addParameter(
    ...
    def shortHelpString(self):
        return self.tr('This Algorithm takes a source layer containing a GeoJSON as a String in a field and creates a copy of this layer with the geometry of this field')